...
sub make_hhm_hhblits {
	...
	my $hhblits_path = '/work/brunette/src/hhsuite/hhsuite-2.0.2/bin/';
	...
	my $job = Rosetta::Job->new(
		...
		args         => [
			"-i $fasta_fn",
			"-o $hhblits_out",
			"-d /work/brunette/src/hhsuite/hhblits_database/uniprot20",
			"-ohhm $hhm_file",
			"-oa3m $a3m_file",
			"-n $numb_rounds",
			"-nopred",
			...
			"-cpu $options->{n_procs}",
			"-realign",
			"-psipred ~/src/psipred2.6/bin",
			"-psipred_data ~/src/psipred2.6/data"
],
		dir          => '.',
...
sub run_NN_filter {
	...
	my $a3m_file_no_path = "$fasta_id.a3m"; 
