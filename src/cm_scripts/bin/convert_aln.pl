...
INFILE: foreach my $infile (@ARGV) {
	...
	foreach my $aln (@$alns) {
		...
		if ( $options{max_template_pct_id} < 1 ) {
			...
			$add = ( $add && $pct_id <= $options{max_template_pct_id} );
		}
...
if ( $options{'sort'} ) {
	...
	} elsif ($combined_alns[0]->get_e_value) {
		@combined_alns = sort { $a->get_e_value <=> $b->get_e_value } @combined_alns;
	} else {
		@combined_alns = sort { $b->get_percent_identity <=> $a->get_percent_identity } @combined_alns;
	}
}
...
ALN: foreach my $aln (@combined_alns) {
	if ( $options{renumber} ) {
		$aln->template_name(
			join '_', ( substr($aln->template_name,0,5), $aln_count+$options{renumber_offset} )
		);
	}
